...
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
static inline int susfs_inode_permission(struct inode *inode,
					 unsigned int flags)
{
	int error;
	int mask = MAY_EXEC;
...
static int link_path_walk(const char *name, struct nameidata *nd)
{
	...
	for(;;) {
		...
		if (dentry->d_inode &&
			...
			err = susfs_inode_permission(dentry->d_parent->d_inode,
						     nd->flags);
			if (err) {
				return err;
			}
	...
	*opened &= ~FILE_CREATED;
	...
	if (dir_inode->i_op->atomic_open) {
		...
		if (dentry && !IS_ERR(dentry) && dentry->d_inode) {
			if (susfs_is_inode_sus_path(dentry->d_inode)) {
				error = susfs_inode_permission(dir_inode,
							       nd->flags);
				if (error) {
					return error;
				}
		   ...
		   int *opened)
{
	...
	if (dir->d_inode) {
		if (susfs_is_inode_sus_path(dir->d_inode)) {
			error = susfs_inode_permission(dir->d_parent->d_inode,
						       nd->flags);
			if (error) {
				return error;
			}
...
finish_lookup:
	...
	if (dentry && !IS_ERR(dentry) && dentry->d_inode) {
		if (susfs_is_inode_sus_path(dentry->d_inode)) {
			error = susfs_inode_permission(
				dentry->d_parent->d_inode, nd->flags);
			if (error) {
				error;
			}
			return -ENOENT;
...
#ifdef CONFIG_KSU_SUSFS_OPEN_REDIRECT
	if (!IS_ERR(filp) &&
	    unlikely(filp->f_inode->i_state & INODE_STATE_OPEN_REDIRECT) &&
